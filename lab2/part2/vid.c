...
int dchar(unsigned char c, int x, int y)
{
  ...
  for (r=0; r<32; r++){         // each ASCII char is a 16row x 8col bitmap
    if (r % 2 == 0)
      byte = *(caddress + r/2);     // get each byte of a row

    for (bit=0; bit<16; bit++){  // for each bit in this byte 
      if (byte & (1<<(bit/2)))      // set pixel by 1 bits in this byte
      {
...
{
   int x, y;
   x = co*16;           // pixel location (x,y) in 480x640 matrix
   y = ro*32;
   dchar(c, x, y); 
}
